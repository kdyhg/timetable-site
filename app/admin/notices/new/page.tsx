...
function NoticeForm() {
  ...
  return <>
    ...
    <div className="mx-auto max-w-3xl notion-card space-y-5 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-2"><label className="space-y-1.5 text-sm font-medium"><span>분류</span><select value={form.category} onChange={(event) => set("category", event.target.value)} className="notion-input w-full font-normal">{noticeCategories.map((item) => <option key={item}>{item}</option>)}</select></label><label className="flex items-end gap-2 pb-2 text-sm"><input type="checkbox" checked={form.is_important} onChange={(event) => set("is_important", event.target.checked)} /> 홈 상단 중요 공지로 고정</label></div>
