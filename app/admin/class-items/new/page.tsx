...
function ClassItemForm() {
  ...
  return <>
    ...
    <div className="mx-auto max-w-3xl notion-card space-y-5 p-4 sm:p-6">
      <div className="grid gap-4 lg:grid-cols-2"><label className="space-y-1.5 text-sm font-medium"><span>유형</span><select value={form.item_type} onChange={(event) => set("item_type", event.target.value)} className="notion-input w-full font-normal">{classItemTypes.map((item) => <option key={item}>{item}</option>)}</select></label><label className="space-y-1.5 text-sm font-medium"><span>과목</span><input value={form.subject} onChange={(event) => set("subject", event.target.value)} className="notion-input w-full font-normal" placeholder="예: 수학Ⅱ" /></label></div>
