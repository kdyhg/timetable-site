...
export default function CareerGuidesPage() {
  ...
  return <>
    ...
    <div className="notion-card mb-6 flex flex-col gap-3 p-4 md:flex-row">
      ...
      <div className="flex gap-1 overflow-x-auto">{(["전체", ...careerResourceCategories] as Filter[]).map((item) => <button key={item} type="button" onClick={() => setFilter(item)} className={`min-h-11 shrink-0 rounded-md px-3 py-2 text-sm font-medium ${filter === item ? "bg-[#e9e9e7]" : "text-[#787774] hover:bg-[#f3f3f2]"}`}>{item}</button>)}</div>
    </div>
