...
function SearchContent() {
  ...
  return <>
    <PageHeader title="통합 검색" description="공지, 일정, 진학자료와 대학·학과 정보를 한 번에 검색하세요." crumbs={[{ label: "홈", href: "/" }, { label: "통합 검색" }]} />
    <form onSubmit={(event) => { event.preventDefault(); run(query); }} className="notion-card mb-6 flex gap-2 p-4"><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} className="notion-input flex-1" placeholder="검색어를 입력하세요" /><button className="notion-button notion-button-primary" type="submit"><Search className="h-4 w-4" /> 검색</button></form>
